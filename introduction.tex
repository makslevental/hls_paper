...
\commnt{more examples}{Similarly, }X-ray crystallography employs high-energy diffraction microscopy techniques, which can sample at up to 1~MHz\cite{doi:10.1063/5.0006531}.
...
%Seems important to note that BraggNN is faster than physics-based methods such as pseudo-Voigt by a large factor (100?). Otherwise the reader may not appreciate why DNNs are being used. The text as currently written could lead readers to conclude that 1) people are using DNNs, for reasons not stated; 2) but DNNs are really slow, so we need special hardware. 
...
Deep neural networks (DNNs), effective in many other academic and commercial domains, have recently been considered for these real-time scientific use cases~\cite{Guest:2018yhq}.
...
\ian{Next seems wordy: can say less about ASICs?}
...
One of the principal challenges in deploying DNNs to FPGAs and ASICs is translating the high-level (with respect to abstraction) representations that DNNs are typically represented as, to the Register Transfer Level (RTL) representations which can be used for FPGA and ASIC design.
