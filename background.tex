...
Given a PyTorch model \inlinepython{model} and a set of example inputs \inlinepython{x}, producing a TS representation only requires calling \inlinepython{torch.jit.trace(model, (x,))}.
...
Our entrypoint into this compiler framework is the \inlinemlir{torch} dialect~\cite{torch-mlir}, a high-fidelity mapping from TS IR to MLIR native IR, which, in addition to performing the translation to MLIR, provides for us shape refinement (mentioned as necessary above).

While \inlinemlir{torch} dialect acts as a thin shim around TS IR and does little "heavy lifting", the same cannot be said for other dialects in MLIR.
For example, the \inlinemlir{linalg} dialect is designed to address the hierarchical optimization problem, wherein the goal is to enable code generation of efficient code or dispatch to existing, previously optimized, kernel code, without sacrificing ease of use and performance for either path.
...
We make extensive use of the linalg dialect as an intermediary between lower-level dialects, such as the \inlinemlir{affine} and \inlinemlir{scf} (structured control flow) dialects, and \inlinemlir{torch} dialect.
The \inlinemlir{scf} is a straightforward formalization of control flow primitives, such as conditionals and loops, so we do not discuss it in great detail.
...
\begin{enumerate}
	\item HLS schedules operations (such as \inlinecpp{fmul}, \inlinecpp{fadd}, \inlinecpp{load}, \inlinecpp{store}) in order to determine which operations should occur during each clock-cycle. Such a schedule depends on three characteristics of the high-level representation:
	      \begin{itemize}
