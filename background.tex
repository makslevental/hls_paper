...
With respect to the kind of program analysis necessitated by our attempted translation, there is another cost to define-by-run DNN specification: the dataflow graph (DFG) is \commnt{cite the pytorch paper that says this explicitly https://openreview.net/pdf?id=BJJsrmfCZ ¶ instead, every intermediate result records only the subset of the computation graph that was ¶ relevant to their computation.}{never fully materialized} and the control flow graph (CFG) is difficult to extract from the semantics of the general purpose language (Python in the case of PyTorch).
...
The exact operation of this tracing mechanism is \commnt{cite dispatcher readme}{beyond the scope of our work}, but two of its limitations, as they pertain to our work, merit discussion.
Firstly, much like other JIT compilers, the TS JIT does not\commnt{in fact it does (if while then primitives) but you have to write those yourself}{ effectively support control flow in the DNN model} specification.
...
In addition, PyTorch also supports at least \commnt{citation for definition}{two kernel fusion} tools, proprietary (NNC fuser) and third party (nvfuser).
...
\commnt{citation}{Multi-level Intermediate Representation} (MLIR) is a new approach to building reusable and extensible compiler infrastructure.
...
Our entrypoint into this compiler stack is the \commnt{citation}{Torch dialect }, a high-fidelity mapping from TS IR to MLIR native IR, that provides for us the necessary shape refinement mentioned above, and various other translations that reduce the impedance mismatch between TS and MLIR.
...
The affine dialect, on the other hand, provides a formalization of semantics that lend themselves to \commnt{cite}{polyhedral compilation techniques}, i.e., techniques that make dependence analysis and loop transformations efficient and reliable.
...
In particular, Xilinx's Vitis HLS, based on the \commnt{cite autopilot \url{https://link.springer.com/chapter/10.1007/978-1-4020-8588-8_6}}{Autopilot project}, recently enabled passing LLVM IR to the tool, rather than C/C++.
...
High-level synthesis tools produce RTL descriptions of digital designs from high-level representations, such as \commnt{cite alternatives}{C or C++}, or in the case of Vitis HLS, LLVM IR.
...
\end{enumerate}
In general, the scheduling problem can be expressed as an \commnt{cite julian}{integer programming problem}
\begin{figure}
...
\begin{figure}
  ...
  \caption{This is a placeholder.}
\end{figure}
which is amenable to Bellman-Ford $O(n^2 + mn)$\commnt{this is phrased poorly - unimodular guarantees you integral solutions to the LP but also gives you the shortest path analogy property}{ for checking consistency of a solution (essentially the shortest path problem) }and an \commnt{\url{https://www.csl.cornell.edu/\~zhiruz/pdfs/sdc-dac2006.pdf}}{LP approach for optimizing the critical path}.

In addition to fulfilling these three fundamental tasks, High-level synthesis tools such as Vitis, \commnt{\url{https://ieeexplore.ieee.org/document/9586110}}{Panda}, \commnt{\url{https://dl.acm.org/doi/10.1145/2514740}}{LegUp} perform standard compiler optimization passes on the IR (that they either receive or produce internally).
...
Recently, open source alternatives for certain FPGAs have become available, thanks to herculean efforts made to reverse engineer the various bitstream formats of, for example, \commnt{cite}{some of Xilinx's architectures}, and reimplement logic synthesis in open source.
Namely, \commnt{cite}{Yosys Open SYnthesis Suite} is a framework for Verilog RTL synthesis.
