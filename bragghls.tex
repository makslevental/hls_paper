...
\begin{abstract}
In many experiment-driven scientific domains, such as X-ray science, high-energy
physics, material science, and cosmology, high data rate experiments
impose hard constraints on data acquisition systems:
collected data must either be stored indiscriminately for post-processing
and analysis, thereby necessitating large storage capacity, or
filtered accurately in real-time, thereby necessitating low-latency processing.
...
design and deployment difficulties. This paper presents a lightweight open-source compiler framework, \texttt{BraggHLS}, based on high-level
...
the context of high-energy diffraction microscopy. We show that \texttt{BraggHLS}
can produce a network implementation with a throughput of just 4.8 \textmu s/sample, an approximately 4$\times$ improvement
over an implementation on a state-of-the-art GPU that could not be deployed at the instrument.
...
for DNNs, until recently, have not been performant enough for
...
both memory (for weights) and compute (floating-point arithmetic),
...
\cite{https://doi.org/10.48550/arxiv.1603.04467}, and MXNet~\cite{https://doi.org/10.48550/arxiv.1512.01274}),
which abstract all implementation details, thereby making
portability of existing model architectures (e.g., to FPGA) close to impossible without rewriting the framework.
\end{enumerate}
...
MLIR~\cite{https://doi.org/10.48550/arxiv.2002.11054}, Glow~\cite{https://doi.org/10.48550/arxiv.1805.00907})
have demonstrated the ability to reduce dramatically inference DNN latencies
\cite{https://doi.org/10.48550/arxiv.1809.02697}, training times
\cite{9664259}, and memory usage~\cite{https://doi.org/10.48550/arxiv.1604.06174}. These compilers function by extracting intermediate-level
representations (IRs) of the DNNs from the representations produced
by the frameworks, and performing various optimizations
(e.g., kernel fusion~\cite{10.1145/2858788.2688521}, vectorization
\cite{maleki2011evaluation}, memory planning~\cite{https://doi.org/10.48550/arxiv.1604.06174}) on those IRs.
...
compiler and HLS framework that can lower DNNs defined as PyTorch
...
describes a
case study in which \texttt{BraggHLS} is applied to \texttt{BraggNN}, a
Bragg peak detection DNN with a target latency of 1 \textmu s/sample.
Finally, Section~\ref{sec:Conclusion} concludes and discusses
...
and resources);
\item The delay for each operation; and
...
\texttt{BraggHLS} is a compiler and HLS framework that employs MLIR
...
Figure~\ref{fig:BraggHLS-framework-overview.-3} shows its overall architecture.
\begin{figure}[tbh]
\centering{}\includegraphics[width=0.7\columnwidth]{figures/BraggHLS}\caption{\texttt{BraggHLS} framework overview.\label{fig:BraggHLS-framework-overview.-3}}
\end{figure}
...
Listing~\ref{lis:Single-filter-convolution-2-1}) and indeed \texttt{BraggHLS}
...
We now discuss some aspects of \texttt{BraggHLS} in more detail.
...
forwarding using MLIR's \texttt{-affine-scalrep} pass. Note, the
longest time is 577419 s$\div$3600$\approx$160 hours (for a loop nest
with a trip count of 128$\times$128$\times$3$\times$3=147,456). \label{fig:-kernel-convolution-full}}
...
so as to determine array index operands of all stores
and loads, such as \mintinline[fontsize={\small}]{mlir}!memref.load %input[%i1,%i5,%i3,%3,%4]!,
and thereupon perform memory dependence checks, thus transforming
...
\item \textbf{Hoist globals}: Move fixed DNN tensors (i.e., weights) out of the body of the generated Python function\footnote{\texttt{BraggHLS} translate the MLIR \texttt{module} corresponding
...
an arbitrary DNN involves formulating and solving an ILP. In the resource-unconstrained
...
\end{minted}
\caption{Parallel loop representation of convolution in Listing~\ref{lis:Single-filter-convolution}.\label{lis:Single-filter-convolution-2-2}}
\end{listing}
...
where \emph{WNS} is the worst negative slack reported). Note, in the case
...
\end{figure*}
\begin{figure*}[tbh]
\centering{}\includegraphics[width=1\columnwidth]{figures/elapsed_time}\caption{Vitis HLS vs. \texttt{BraggHLS} runtime vs. unroll factor.\label{fig:Runtime-of-Vitisa}}
\end{figure*}
...
High-energy diffraction microscopy techniques enable non-destructive
...
Lorentzian, Voigt, or Pseudo-Voigt. As noted in Section
\ref{sec:Introduction}, HEDM experiments can have collection rates
in excess of 80 GB/s. These data rates, though more modest than at the LHC, combined with the high cost of the fitting procedure,
...
\end{minted}
\caption{\texttt{BraggNN} for scale \emph{s=1,2}.\label{lis:braggnn}}
\end{listing}
...
\textmu s/sample (via batch inference) on a state-of-the-art GPU: a large speedup over classical pseudo-Voigt peak fitting methods, but still far
...
we applied \texttt{BraggHLS} to the PyTorch representation of \texttt{BraggNN(\emph{s}=1)}\emph{
...
longest stage measuring 480 intervals, for  a throughput of 4.8 \textmu s/sample. See Table~\ref{tab:Resource-usage-for}
...
We reduced the precision from half precision to FloPoCo (5,4)-precision
...
divide the bit width of FloPoCo (5,4)-precision cores.
(The actual width for FloPoCo (5,4)-precision is 12 bits: 1 extra bit is needed for the sign and 2 for FloPoCo's handling of exceptional conditions.)
...
needs 23,328 SLLs between SLR2 and SLR1\footnote{We route the output of \texttt{cnn\_layers\_1} (1$\times$16$\times$9$\times$9$\times$12
wires) as well as the output of \texttt{soft(theta\_layer}$\times$\texttt{phi\_layer)}$\times$\texttt{g\_layer}
(1$\times$8$\times$9$\times$9$\times$12 wires) from SLR2 to SLR1.}. Thus, we further reduced the precision to (5,3). Finally,
...
``latch'' registers in each SLR. Figure~\ref{fig:BraggHLS-framework-overview.-2} illustrates the effect of using launch and latch registers as well as placement and routing constraints.
\begin{figure*}[tbh]
...
we propose future work that can further improve on this result.

\section{Conclusion\label{sec:Conclusion}}

This paper presents \texttt{BraggHLS}, an MLIR-based HLS compilation
framework, which supports translating DNN models to RTL without any
reliance on commercial HLS tools. \texttt{BraggHLS} enables an end-to-end
compilation pipeline by providing a PyTorch front-end and a Verilog
emission back-end. \texttt{BraggHLS} also contains extensible Python
intermediate layer, in order to support use-case optimizations. Using
the optimization functionality, \texttt{BraggHLS} is able to support
some optimizations (namely, \texttt{store}-\texttt{load} forwarding)
not possible otherwise. Experimental results demonstrate that \texttt{BraggHLS}
...
Several existing frameworks aim to accomplish adjacent or similar goals (namely, translating DNN models to RTL).
They fall into two categories: those that rely on a commercial HLS tool for the scheduling and RTL emission~\cite{daceml, Duarte:2018ite, 9786533} and those that do not~\cite{7783720, takamaeda2015pyverilog}. As we've already observed, for those tools that rely on commercial HLS tool, it is difficult to efficiently explore the design space and attain low latencies due to the formal correctness guarantees that the HLS tool is bound by. Effectively the commercial HLS tool becomes a narrow bottleneck. Conversely, for those frameworks that do not rely on commercial HLS, we observe that they make strong assumptions on the workloads that; both DNN Weaver~\cite{7783720} and NNGen~\cite{takamaeda2015pyverilog} produce RTL designs that emulate GPU architectures and thus optimize for throughput rather than end-to-end latency.

\subsection{Future work\label{subsec:futurework}}

\texttt{BraggHLS} leaves several directions for future work, primarily with respect to scheduling:

\begin{itemize}
    \item Better integration between the Python layer and MLIR: it is preferable that the transformations on the Python representation could make use of various MLIR facilities, such as affine analysis, for the purposes of exploring loop transformations that improve latency;
    \item Expanding the set of scheduling algorithms available: for example, resource aware scheduling~\cite{10.1145/3174243.3174268};
    \item Integration of scheduling-aware placement and vice-versa (placement-aware scheduling): currently \texttt{BraggHLS} can be used to inform placement but does not explicitly emit placement constraints (see Section~\ref{sec:BraggNN-case-study}); a more precise approach, such as in~\cite{10.1145/3431920.3439289}, would potentially enable better pipelining and thus higher throughput.
\end{itemize}
...
\begin{table*}[tbh]
\caption{Resource usage for \texttt{BraggNN}(\emph{s}\/=1) and (5,3)-precision
