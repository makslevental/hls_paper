...
\maketitle
\renewcommand{\MintedPygmentize}{/home/mlevental/dev_projects/hls_paper/scripts/pygmentize.py}
...
be \emph{behaviorally verified} (see Section \ref{subsec:Symbolic-execution-for}).
...
full unrolling time vs. input (square) image size, with store-load
...
the total schedule interval count, we report the best possible interval
count (\texttt{LatencyBest} from the Vitis HLS reports). All other
measurements are collected from Vivado synthesis reports. Note, since
Vitis HLS operates on C++ representations, we generate such a representation
for our test cases by first lowering each to the \texttt{affine} dialect
and then using the \texttt{scalehls-translate} tool of the ScaleHLS
project \cite{yehpca2022scalehls} to emit C++. Note further, we do
not make any use of \texttt{scalehls-opt} tool (of the same project). 
...
unroll the representation that's passed to Vitis HLS. Thus, all Vitis
...
\end{itemize}
The layers were chosen to cover a range of arithmetic operations and
data access patterns, i.e., iteration, accumulation, and reduction.
The concrete parameter values and dimensions of inputs used during
evaluation are summarized in Table \ref{tab:Resource-usage-for-2}.
\begin{table*}[tbh]
...
either through store-load forwarding or further array partitioning\footnote{Indeed Vitis HLS can be seen to produce warnings such as \colorbox{bg}{\parbox{\columnwidth}{\texttt{WARNING: [HLS 200-885] Unable to schedule 'load' operation ...
due to limited memory ports (II = 1). Please consider using a memory core with more ports or partitioning the array ...}}}}. Conversely, \texttt{BraggHLS}'s ability to effectively perform store-load
...
timing, i.e., while the interval count is decreased in the clock period
is increasing (see Appendix Table \ref{tab:all_unrolls} for exact
values). 
...
Bragg diffraction peaks, achieves a batched latency of approximately
22 \textmu s/sample. This is a large speedup over the classical pseudo-Voigt
peak fitting methods, but still falls far short of the 1 \textmu s/sample
target for handling the 1 MHz sampling rates. In addition, the current
implementation of \texttt{BraggNN}, deployed to either a data-center
class GPU such as a NVIDIA V100, or even a workstation class GPU such
as a NVIDIA RTX 2080Ti, has no practicable means to being deployed
at the edge, i.e., adjacent or proximal to the high energy microscopy
equipment. Towards this end, we applied \texttt{BraggHLS} to the PyTorch
representation of \texttt{BraggNN($s=1$)}\emph{ }(see Listing \ref{lis:braggnn})
and achieved a RTL implementation which synthesizes to a 1,238 interval
...
stage pipeline with the longest stage measuring 480 intervals. Thus,
the throughput of the implementation is 4.8 \textmu s/sample. See
Table \ref{tab:Resource-usage-for} for a summary of the resource
usage of the implementation and Figure \ref{fig:braggnn} for a comparison
with designs generated by Vitis HLS.
...
compile, synthesize, place and route \texttt{BraggNN} to Xilinx's
Alveo U280 at a throughput of 4.8 \textmu s/sample. This is \textasciitilde 5$\times$
higher latency than the target 1 \textmu s/sample but it is a 4$\times$
