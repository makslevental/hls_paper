...
\title[\texttt{BraggHLS}]{\texttt{BraggHLS}: High-level Synthesis for low-latency Deep Neural Networks for experimental science}
...
diffraction microscopy (HEDM) techniques (e.g., Bragg peak diffraction analysis), which provide non-destructive
...
for DNNs, are not performant enough for
...
portability of model architectures (e.g., to FPGA) close to impossible without rewriting the framework.
\end{enumerate}
...
that can effectively transform unoptimized, hardware-agnostic PyTorch
models into low latency RTL designs suitable for deployment to Xilinx
FPGAs. \texttt{BraggHLS} is thoroughly tested and open source\footnote{Available
at REMOVED FOR DOUBLE BLINDING};
...
\texttt{BraggHLS} targets the PyTorch framework.
...
a priori, the runtime-materialized DFG is only partially materialized,
...
\label{subsec:High-level-synthesis}
...
(a) The topological ordering of the DFG/CFG of the procedural representation
...
\section{Compiler and HLS framework\label{sec:BraggHLS-compiler-and}}
...
\end{figure}
\texttt{BraggHLS} first lowers DNNs from PyTorch to
...
achieving the target latency, since it often took a prohibitive amount
of time.
\begin{figure}[tbh]
\begin{centering}
...
\item \textbf{Hoist globals}: Move fixed DNN tensors (i.e., weights) out of the body of the generated Python function\footnote{\texttt{BraggHLS} translates the MLIR \texttt{module} corresponding
...
Listing~\ref{lis:Single-filter-convolution}.\label{lis:Single-filter-convolution-2-2}}
\end{listing}
...
\begin{figure}[tbh]
\centering{}\includegraphics[width=1\columnwidth]{figures/elapsed_time}\caption{Vitis HLS vs. \texttt{BraggHLS} runtime vs. unroll factor.
...
by \texttt{BraggHLS}. Even at an unroll factor of 1024 (which corresponds
...
The increasing latency (as a function of unroll factor) in th \texttt{max\_pool\_2d} case is due to Vitis HLS's failure to meet
...
latency vs. unroll factor (with both at half-precision).\texttt{\label{fig:braggnn}}}
\end{figure}
...
\ref{sec:Introduction}, HEDM experiments can collect data
at more than 80 GB/s. These data rates, though more modest than at the LHC, combined with the high cost of the fitting procedure,
...
longest stage measuring 480 intervals, for  a throughput of 4.8 \textmu s/sample. See Figure \ref{fig:braggnn} for a comparison with designs generated by Vitis
HLS.
...
needs 23,328 SLLs between SLR2 and SLR1\footnote{We route from SLR2 to SLR1 the outputs of \texttt{cnn\_layers\_1} (1$\times$16$\times$9$\times$9$\times$12
wires) and \texttt{soft(theta\_layer}$\times$\texttt{phi\_layer)}$\times$\texttt{g\_layer}
(1$\times$8$\times$9$\times$9$\times$12 wires).}. Thus, we further reduced the precision to (5,3). Finally,
...
% Figure~\ref{fig:BraggHLS-framework-overview.-2} illustrates the effect of using launch and latch registers as well as placement and routing constraints.
% \begin{figure*}[tbh]
...
We have presented \texttt{BraggHLS}, an MLIR-based HLS compilation
framework that supports translating DNN models to RTL without the use of
commercial HLS tools. The \texttt{BraggHLS} end-to-end
...
Other frameworks that aim to
translate DNN models to RTL fall into two categories: those that rely on a commercial HLS tool for scheduling and RTL emission~\cite{daceml, Duarte_2018, 9786533} and those that do not~\cite{7783720, takamaeda2015pyverilog}. The former cannot easily explore the design space efficiently to attain low latencies, due to the formal correctness guarantees that the HLS tool is bound by; in effect, the commercial HLS tool becomes a narrow bottleneck. Conversely, for the latter, we observe that they make strong assumptions on the workloads that they address; both DNN Weaver~\cite{7783720} and NNGen~\cite{takamaeda2015pyverilog} produce RTL designs that emulate GPU architectures and thus optimize for throughput rather than end-to-end latency.
...
(2) Expanding the set of scheduling algorithms available: for example, resource aware scheduling~\cite{10.1145/3174243.3174268}; abd
