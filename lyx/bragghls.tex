...
Figure TODO). Translating \texttt{scf} dialect to python enables \texttt{BraggHLS}
...
\begin{enumerate}
\item Partially evaluate functions of iteration variables, such as \mintinline{mupad}!%3 = arith.addi %iv3, %iv6!,
which enables concretely determining array index operands of all stores
and loads, such as \mintinline{mupad}!memref.load %input[%iv1,%iv5,%iv3,%3,%4]!,
...
{\small{}$\left\llbracket \texttt{memref<}b\times c_{in}\times h\times w\texttt{>}\right\rrbracket $} &  & {\small{}$\texttt{MemRef(}b\times c_{in}\times h\times w\texttt{)}$}\tabularnewline
{\small{}$\left\llbracket \texttt{\%5 = memref.load input[\%iv1, \%iv5, \%3, \%4]}\right\rrbracket $} &  & {\small{}$\texttt{v5 = input.\_\_getitem\_\_((iv1, iv5, v3, v4))}$}\tabularnewline
{\small{}$\left\llbracket \texttt{memref.store \%9, output[\%iv1, \%iv5, \%3, \%4]}\right\rrbracket $} &  & {\small{}$\texttt{output.\_\_setitem\_\_((iv1, iv5, v3, v4), v9)}$}\tabularnewline
{\small{}$\left\llbracket \texttt{scf.for \%iv1 = \%c0 to }b\texttt{ step \%c1}\right\rrbracket $} &  & {\small{}$\texttt{for iv1 in range(c0,}b\texttt{,c1)}$}\tabularnewline
{\small{}$\left\llbracket \texttt{\%3 = arith.addi \%iv3, \%iv6}\right\rrbracket $} &  & {\small{}$\texttt{\texttt{v3 = iv3 + iv6}}$}\tabularnewline
{\small{}$\left\llbracket \texttt{\%8 = arith.mulf \%5, \%6}\right\rrbracket $} &  & {\small{}$\texttt{\texttt{v8 = v5.\_\_mul\_\_(v6})}$}\tabularnewline
{\small{}$\left\llbracket \texttt{\%9 = arith.addf \%7, \%8}\right\rrbracket $} &  & {\small{}$\texttt{\texttt{v9 = v7.\_\_add\_\_(v8)}}$}\tabularnewline
\vspace{-15pt}
 &  & \tabularnewline
{\small{}$\left\llbracket \begin{array}{c}
\texttt{\%63 = arith.cmpfugt \%10, \%c0}\\
\texttt{\%64 = arith.select \%63, \%c0\,\,}
\end{array}\right\rrbracket $} &  & {\small{}$\texttt{\texttt{v64 = v10.relu()}}$}\tabularnewline
\vspace{-10pt}
 &  & \tabularnewline
{\small{}$\left\llbracket \begin{array}{c}
\texttt{\texttt{\%8 = arith.mulf \%5, \%6}}\\
\texttt{\%9 = arith.addf \%7, \%8}
\end{array}\right\rrbracket $} &  & {\small{}$\texttt{\texttt{v9 = fma(v5, v6, v7)}}$}\tabularnewline
\vspace{-10pt}
 &  & \tabularnewline
\bottomrule
...
corresponding to unique values of the iteration variables (e.g., \texttt{\%iv1},\texttt{
\%iv2},\texttt{ \%iv3},\texttt{ \%iv4} for Listing \ref{lis:Single-filter-convolution-2-2})
...
\centering{}\label{2dlattice-1-3}\subfloat[\texttt{conv}]{\centering{}\includegraphics[width=1\columnwidth]{../data/conv}\label{2dlattice-1-1-1-1}}\subfloat[\texttt{max\_pool\_2d}]{\centering{}\includegraphics[width=1\columnwidth]{../data/max_pool_2d}\label{2dlattice-1-2-1-1}}\caption{Resource usage and latency vs. unroll factor of various DNN modules.}
\end{figure*}
\begin{figure*}[tbh]
...
for just ops).
\begin{figure}[tbh]
\centering{}\includegraphics[width=1\columnwidth]{/home/mlevental/dev_projects/hls_paper/data/braggnn}\caption{\texttt{braggnn}}
\end{figure}
