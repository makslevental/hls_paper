...
\item We discuss some of the challanges faced after even after successful
synthesis of RTL from a high-level representation of a DNN, namely
we present a case study DNN RTL implementation and discus the further
challanges faced during deployment to FPGA.
\end{enumerate}
...
Critically, and distinctly, it handles the DNN transformations as
well as scheduling, binding, and FSM extraction; there is no dependence
on any commercial HLS tools. Figure \ref{fig:BraggHLS-framework-overview.-3}
shows the architecture of \texttt{BraggHLS} 
\begin{figure}[tbh]
\centering{}\includegraphics[width=0.7\columnwidth,bb = 0 0 200 100, draft, type=eps]{../figures/BraggHLS.pdf}\caption{\texttt{BraggHLS} framework overview (placeholder).\label{fig:BraggHLS-framework-overview.-3}}
\end{figure}
. 
...
The \texttt{scf} dialect describes standard control flow primitives,
such as conditionals and loops, and is mutually compatible with the
\texttt{arith} and \texttt{memref} dialects. Such a representation
lends itself to a straightforward translation to python (compare Listing
\ref{lis:Single-filter-convolution} to Listing \ref{lis:Single-filter-convolution})
and indeed \texttt{BraggHLS} performs this translation.
\begin{listing}
...
@conv2d(
  ...
  scf.for %iv1 = %c0 to |$b$| step %c1 {
    scf.for %iv2 = %c0 to |$c_{out}$| step %c1 {
      scf.for %iv3 = %c0 to |$h$| step %c1 {
        scf.for %iv4 = %c0 to |$w$| step %c1 {
...
that can be scheduled efficiently using the scheduling infrastructure
in CIRCT \cite{oppermann2022eurollvm} (an MLIR adjacent project)
and then emits corresponding RTL (as Verilog). 

\texttt{BraggHLS} delegates to the FloPoCo \cite{8877424} hardware
the task of generating pipelined implementations of the standard floating
point arithmetic operations (\texttt{mul}, \texttt{div}, \texttt{add},
\texttt{sub}, \texttt{sqrt}) at various precisions. In addition, we
implement a few generic (parameterized by bit width) operators in
order to support a range of DNN operations: two-operand maximum (\texttt{max});
negation (\texttt{neg}); rectified linear units (\texttt{relu}). Transcendental
functions, such as \texttt{exp}, are implemented using a Taylor series
expansion to $k$-th order (where $k$ is determined on a case by
case basis). Note, FloPoCo's floating point representation differs
...
We now discuss the benefits of translating \texttt{scf} dialect to
python.

\subsection{Symbolic interpretation for fun and profit\label{subsec:Symbolic-execution-for}}

As discussed in Section \ref{subsec:High-level-synthesis}, a loop-nest
representation of a DNN enables us to effectively perform data-flow
analysis and schedule the encompassed arithmetic operations, given
that we can first unroll the loops. As also discussed in Section \ref{subsec:High-level-synthesis},
the formally correct approach to unrolling a loop nest is prohibitively
expensive in terms of runtime. Indeed, for example, in the case of
\texttt{BraggNN}, repeatedly performing this unrolling was a hindrance
to effectively searching the design space for a RTL representation
achieving the target latency, since it often took an enormous amount
of time (see Figure TODO). Translating \texttt{scf} dialect to python
enables \texttt{BraggHLS} to overcome this barrier by enabling a \emph{symbolic
interpreter} for the loop-nest representations. Specifically, after
the translation, and after applying some transformations on the resulting
python abstract syntax tree (AST), the python code can be executed,
with operations on python values determined by class method overloads
(see Algorithm \ref{alg:scf-dialect-to}). 

{\scriptsize{}}
\begin{algorithm}[tbh]
{\scriptsize{}\caption{\texttt{scf} dialect to python translation.\label{alg:scf-dialect-to}}
}{\scriptsize\par}
\begin{lyxcode}
{\scriptsize{}
\begin{multline*}
\left\llbracket \texttt{\%5}\right\rrbracket \rightarrow\texttt{\texttt{v5 = Val("\%5")}}\\
\end{multline*}
\vspace{-30pt}
\begin{multline*}
\left\llbracket \texttt{memref<}b\times c_{in}\times h\times w\texttt{>}\right\rrbracket \rightarrow\texttt{MemRef(}b\times c_{in}\times h\times w\texttt{)}
\end{multline*}
\vspace{-20pt}
}{\scriptsize\par}
...
{\scriptsize{}
\begin{multline*}
\left\llbracket \texttt{\%5 = memref.load input[\%iv1,\%iv5,\%3,\%4]}\right\rrbracket \\
\rightarrow\texttt{v5 = input.\_\_getitem\_\_((iv1,iv5,v3,v4))}
\end{multline*}
\vspace{-20pt}
}{\scriptsize\par}
...
{\scriptsize{}
\begin{multline*}
\left\llbracket \texttt{memref.store \%9, output[\%iv1,\%iv5,\%3,\%4]}\right\rrbracket \\
\rightarrow\texttt{output.\_\_setitem\_\_((iv1,iv5,v3,v4), v9)}
\end{multline*}
}{\scriptsize\par}
...
{\scriptsize{}
\begin{multline*}
\left\llbracket \texttt{scf.for \%iv1 = \%c0 to }b\texttt{ step \%c1}\right\rrbracket \\
\rightarrow\texttt{for iv1 in range(c0,}b\texttt{,c1)}
\end{multline*}
}{\scriptsize\par}
...
{\scriptsize{}
\begin{multline*}
\left\llbracket \texttt{\%3 = arith.addi \%iv3, \%iv6}\right\rrbracket \rightarrow\texttt{\texttt{v3 = iv3 + iv6}}
\end{multline*}
\vspace{-20pt}
}{\scriptsize\par}
...
{\scriptsize{}
\begin{multline*}
\left\llbracket \texttt{\%8 = arith.mulf \%5, \%6}\right\rrbracket \rightarrow\texttt{\texttt{v8 = v5.\_\_mul\_\_(v6})}
\end{multline*}
\vspace{-20pt}
}{\scriptsize\par}
...
{\scriptsize{}
\begin{multline*}
\left\llbracket \texttt{\%9 = arith.addf \%7, \%8}\right\rrbracket \rightarrow\texttt{\texttt{v9 = v7.\_\_add\_\_(v8)}}
\end{multline*}
}{\scriptsize\par}
...
on resource constrained operations: for parallelizable loop-nests,
which are reprepresented as \texttt{scf.parallel} loop-nests (see
Listing \ref{lis:Single-filter-convolution-2-2}) 
\begin{listing}
\begin{minted}[fontsize={\footnotesize},escapeinside={||},mathescape=true]{mupad}
@conv2d(
    %input: memref<|$b \times c_{in} \times h \times w$|>,
...
\end{listing}
 we can partition the DNN across a known set of hardware resources
in order to maximize parallelism; since for each value of (\texttt{\%iv1},\texttt{
...
as striding and padding make this a non-trivial calculation. 

Once peak usage is determined, successive instances \texttt{scf.parallel}
loop bodies evaluated at the same concrete values of (\texttt{\%iv1,
\%iv2, \%iv3, \%iv4}) are bound to the same resources. Then since
successive instances of \texttt{scf.parallel} loop bodies evaluated
at the same concrete values of the iteration variables are bound to
the same resources, all resource constrained operations common to
both instances are ordered. Note, since each loop nest corresponds
to a DNN operation, ordering of\texttt{ scf.parallel} loop nests is
determined by the higher-level structure of the DNN. 

Note, For DNN operations that lower to sequential loop nests (e.g.,
\texttt{sum}, \texttt{max},or \texttt{prod}), we fully unroll the
loops and transform the resulting, sequential, operations to a reduction
tree; we use As-Late-As-Possible scheduling \cite{baruch1996scheduling}
...
\begin{figure*}[tbh]
\begin{centering}
\subfloat[\texttt{addmm}]{\centering{}\includegraphics[width=1\columnwidth]{../data/addmm}\label{2dlattice-1-1-2}}\subfloat[\texttt{braggnn}]{\centering{}\includegraphics[width=1\columnwidth]{../data/batch_norm}\label{2dlattice-1-2-2}}
\par\end{centering}
\medskip{}

\centering{}\label{2dlattice-1-3}\subfloat[\texttt{conv}]{\centering{}\includegraphics[width=1\columnwidth]{../data/conv}\label{2dlattice-1-1-1-1}}\subfloat[\texttt{max\_pool\_2d}]{\centering{}\includegraphics[width=1\columnwidth]{/home/mlevental/dev_projects/hls_paper/data/max_pool_2d}\label{2dlattice-1-2-1-1}}\caption{Resource usage and latency vs. unroll factor of various DNN modules.}
\end{figure*}
\begin{figure*}[tbh]
...
\begin{figure}[tbh]
\centering{}\includegraphics[width=1\columnwidth,bb = 0 0 200 100, draft, type=eps]{../figures/exp_hist.pdf}\caption{\texttt{BraggHLS} weights exponent distribution (placeholder).\label{fig:BraggHLS-framework-overview.-1}}
\end{figure}
